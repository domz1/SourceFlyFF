...
//	CWndButton* pWndButton3 = AppendMenu( this, 0, 0 , GETTEXT( TID_APP_FIND          ) ); pWndButton3->SetTexture( g_Neuz.m_pd3dDevice, MakePath( DIR_ICON, _T( "icon_Folder.dds" ) ) );
	...
	CWndButton* pWndButton4 = AppendMenu( this, 0, APP_OPTIONEX2, GETTEXT(TID_APP_OPTION) );
...
BOOL CWndTaskBar_HUD::Process( void )
{
	...
	if(( g_Neuz.m_bActiveNeuz != FALSE )&&( pWndEdit == NULL) )
	{
		...
		///////////////////1
		if(( GetKeyState( '1' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////2
		if(( GetKeyState( '2' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////3
		if(( GetKeyState( '3' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////4
		if(( GetKeyState( '4' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		///////////////////5
		if(( GetKeyState( '5' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////6
		if(( GetKeyState( '6' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////7
		if(( GetKeyState( '7' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////////8
		if(( GetKeyState( '8' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		else
		{
		  ...
		  /////////////////9
		if(( GetKeyState( '9' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		else
		{
		  ...
		  /////////////////////0
		if(( GetKeyState( '0' ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
...
/*		EXPINTEGER	nExpResult = pMover->GetExp1() * (EXPINTEGER)10000 / pMover->GetMaxExp1();
    ...
    for( int i = 0; i < 10; i++ )
    {
		...
		if((  m_USEFIRE[i] != NULL )&&( !m_aSlotItem[0][i].IsEmpty() ))
        {
	...
	for( int i = 0; i < 10; i++ )
	{
		...
		CD3DFont* pOldFont = p2DRender->GetFont();
		p2DRender->SetFont( CWndBase::m_Theme.m_pFontWndNewTitle );
		p2DRender->TextOut( (1 + (point.x  / point.x + point.x ) ), point.y - 12 , szTemp, D3DCOLOR_ARGB( 255, 246, 204, 77 ) );
		p2DRender->SetFont( pOldFont );
	} 
}
...
void CWndTaskBar_HUD::OnMouseWndSurface( CPoint point )
{
    for(int i=0;i<10;i++)
    {
	   CRect recSUR = GetWindowRect();
	   if( !recSUR.PtInRect( point ))
	   {
	   slot1[i] = NULL;
	   }
	...
	LPWNDCTRL wndEXP_Ctrl = GetWndCtrl( WIDC_CUSTOM14 );
    char cbufExp[16] = {0,};
    ...
    CRect rectEXP = wndEXP_Ctrl->rect;
    if( rectEXP.PtInRect( point ) )
	{
	   CPoint point3 = GetMousePoint();;
	   ClientToScreen( &point3 );
	   ClientToScreen( &rectEXP );
		 	
		if( fExp >= 99.99f )
		    strEdit.Format( "LV: %d \nEXP: %d.%% / 99.99%% ", pMover->GetLevel(), nExpResult, sprintf( cbufExp, "99.99%%" ) );
		else
		    strEdit.Format( "LV: %d \nEXP: %d.%% / 99.99%% ", pMover->GetLevel(), nExpResult, sprintf( cbufExp, "%.2f%%", fExp ) );
			
	  g_toolTip.PutToolTip( 10000, strEdit, rectEXP, point3, 0 );
	}
}
#ifdef __NEW_TASKBAR_V19
/**************************************************************************
...
void CWndTaskBar_HUD_EX3::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if(pWndTB)
	{
		if(pWndTB->b_lock == FALSE)
			m_bPickup = FALSE;//AddWndStyle( WBS_MOVE );
	}
	for( int i = 0; i < 10; i++ )
    {
        if( wndCtrl[i]->rect.PtInRect( point ))
...
{	
	if(g_WndMng.m_pWndMenu && g_WndMng.m_pWndMenu->IsVisible())
	 ...
	 CRect ST_Rect = CRect( 12,  6, 48, 24 );//posi豫o do static1
	 CRect ST_Rect2 = CRect( 12,  390, 48, 408 );//posi豫o do static2
...
{	
	if(g_WndMng.m_pWndMenu && g_WndMng.m_pWndMenu->IsVisible())
	 ...
	 CRect ST_Rect = CRect( 2,  4, 20, 36 );//posi豫o do static1
	 CRect ST_Rect2 = CRect( 384,  4, 390, 36 );//posi豫o do static2
...
void CWndTaskBar_HUD_EX1::OnDraw( C2DRender* p2DRender )
{
    ...
    for( int i = 0; i < 10; i++ )
    {
		...
		if((  m_USEFIRE[i] != NULL )&&( !m_aSlotItem[1][i].IsEmpty() ))
        {
            m_USEFIRE[i]->Render( p2DRender, CPoint( wndCtrl[i]->rect.left, wndCtrl[i]->rect.top), m_USEFireALPHA[i] );
		}
...
void CWndTaskBar_HUD_EX1::OnInitialUpdate( void )
{
	...
	{
		if(pWndTB->b_lock == FALSE)
		   m_bPickup = FALSE;//DelWndStyle( WBS_MOVE );
	}
...
BOOL CWndTaskBar_HUD_EX2::Process( void )
{
	...
	if(( g_Neuz.m_bActiveNeuz != FALSE )&&( pWndEdit == NULL ) )
	{
		...
		///////////////////F1
		if(( GetKeyState( VK_F1 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////F2
		if(( GetKeyState( VK_F2 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////F3
		if(( GetKeyState( VK_F3 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////F4
		if(( GetKeyState( VK_F4 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		///////////////////F5
		if(( GetKeyState( VK_F5 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////F6
		if(( GetKeyState( VK_F6 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		////////////////////F7
		if(( GetKeyState( VK_F7 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		/////////////////////F8
		if(( GetKeyState( VK_F8 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		else
		{
		  ...
		  /////////////////F9
		if(( GetKeyState( VK_F9 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
		...
		else
		{
		  ...
		  /////////////////////F10
		if(( GetKeyState( VK_F10 ) & 0x8000 )&& !(GetKeyState( VK_MENU ) &0x8000 ))
		{
...
void CWndTaskBar_HUD_EX2::OnDraw( C2DRender* p2DRender )
{
    ...
    for( int i = 0; i < 10; i++ )
    {
		...
		if((  m_USEFIRE[i] != NULL )&&( !m_aSlotItem[2][i].IsEmpty() ))
        {
            m_USEFIRE[i]->Render( p2DRender, CPoint( wndCtrl[i]->rect.left, wndCtrl[i]->rect.top), m_USEFireALPHA[i] );
		}
	}
...
{ 
	...
	for(int i=0; i < 6; i++)
	{
      ...
      {
        m_SURF_FireALPHA[i] += 8;

        if( ( m_SURF_FireALPHA[i] > 250 ))
        {
            m_SURF_FireALPHA[i] -= 8;
        }
        if( ( m_SURF_FireALPHA[i] < 80 ))
        {
            m_SURF_FireALPHA[i] += 8;
        }
	  }
	  if( m_bSURF_FireFLASH == FALSE )
      {
        m_SURF_FireALPHA[i] = 0;
	  }
	}
...
void CWndNewSkillSlot::OnMouseWndSurface( CPoint point )
{
	...
	if( bInPoint )	
	{
	   ...
	   if( CTRECT0.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[0] == NULL )
		   {
		       m_SURF_FIRE[0] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
	   ...
	   if( CTRECT1.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[1] == NULL )
		   {
		       m_SURF_FIRE[1] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
		   if(!lpShortcut->IsEmpty())
		   {
		         if( lpShortcut->m_dwShortcut == SHORTCUT_SKILL )
	   ...
	   if( CTRECT2.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[2] == NULL )
		   {
		       m_SURF_FIRE[2] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
	   ...
	   if( CTRECT3.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[3] == NULL )
		   {
		       m_SURF_FIRE[3] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
	   ...
	   if( CTRECT4.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[4] == NULL )
		   {
		       m_SURF_FIRE[4] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
	   ...
	   if( CTRECT5.PtInRect( point ) )
	   {
		   ...
		   if( m_SURF_FIRE[5] == NULL )
		   {
		       m_SURF_FIRE[5] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff.tga" ), 0xffff00ff );
		   }
...
int m_ROTFireALPHA = 255;
int TESTALPHAMASK = 255;
void CWndNewSkillSlot::OnDraw( C2DRender* p2DRender )
{
   ...
   else if (( m_nActionSlot == 0 )&&( g_WndMng.m_pWndTaskBar->m_nExecute == 0 ))
   {
	...
	for( int i = 0; i < MAX_SLOT_QUEUE; i++ )
	{
		...
		if( !lpShortcut->IsEmpty() )
		{
			if( lpShortcut->m_pTexture )
			{
					/*if( ( m_nActionSlot > 0 )&&( g_WndMng.m_pWndTaskBar->m_nExecute > 0 ))
					{
					lpShortcut->m_pTexture->RenderRotate(p2DRender,point,RotateSLOT,TRUE,255,fScal,fScal);
					}
					else
					{
					lpShortcut->m_pTexture->Render( p2DRender,point,CPoint(32,32),255,fScal,fScal);
					}*/
				p2DRender->RenderTextureMASK( point,m_Marronzinha, lpShortcut->m_pTexture, 255, fScal ,fScal );
			}
		}
	}
	...
	{			
		...
		slot2->Render( &g_Neuz.m_2DRender, point,TESTALPHAMASK );
	}
	if(slot5 != NULL)
	{
		point.y = 0;
	...
	if( ( m_nActionSlot > 0 )&&( g_WndMng.m_pWndTaskBar->m_nExecute > 0 ))
	{
	  ...
	  //SlotEffect3->RenderRotate( p2DRender, CPoint( m_wndCustom[0].m_rectClient.left - 10, m_wndCustom[0].m_rectClient.top - 14),RotateSkillEffect/3,1);
	}
}
...
void CWndNewSkillSlot::OnInitialUpdate( void )
{
	...
	slot5        = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "slot05.tga" ), 0xffff00ff );
	SlotRt       = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "slot04.tga" ),0xffff00ff );
	SlotRt2      = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "slot04_2.tga" ),0xffffffff );
	SlotEffect5  = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff5.tga" ),0xffffffff );
	SlotEffect3  = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff3_02.tga" ),0xffffffff );
	SlotEffect2  = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff4.tga" ),0xffffffff );
	m_Marronzinha= CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "slot07.tga" ),0xffffffff );

	m_DROP_Fire  = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "ActionSlotEff2.tga" ), 0xffff00ff );
...
{	
	...
	CWndTaskBar_HUD* pWndTB = (CWndTaskBar_HUD*)g_WndMng.GetWndBase(APP_NEWTASKBAR);
	CRect ST_Rect = CRect( 2,  5, 19, 55 );
	if( PtInRect( ST_Rect, point ) )
	{
		ClientToScreen( &point );
		ClientToScreen( &ST_Rect );
...
void CWndTaskBar_HUD_SLIDE::OnDraw( C2DRender* p2DRender )
{
	...
	for( int i = 0; i < 10; i++ )
    {
      ...
      if( wndCtrl[i]->rect.Width() <= 48 + TANTO )
      {
	      m_SlotALPHA[i] = 0;
      }

      if( GetWindowRect().Width() >= 34 + TANTO + 2)
      {
	       m_SlotALPHA[0] = 255;
      }
      if( GetWindowRect().Width() >= 68 + TANTO + 4)
      {
	      m_SlotALPHA[1] = 255;
      }
      if( GetWindowRect().Width() >= 102 + TANTO + 6)
      {
	      m_SlotALPHA[2] = 255;
      }
      if( GetWindowRect().Width() >= 136 + TANTO + 8)
      {
	      m_SlotALPHA[3] = 255;
      }
      if( GetWindowRect().Width() >= 170 + TANTO + 10)
      {
	      m_SlotALPHA[4] = 255;
      }
      if( GetWindowRect().Width() >= 204 + TANTO + 12)
      {
	      m_SlotALPHA[5] = 255;
      }
      if( GetWindowRect().Width() >= 238 + TANTO + 14)
      {
	      m_SlotALPHA[6] = 255;
      }
      if( GetWindowRect().Width() >= 272 + TANTO + 16)
      {
	      m_SlotALPHA[7] = 255;
      }
      if( GetWindowRect().Width() >= 306 + TANTO + 18)
      {
	      m_SlotALPHA[8] = 255;
      }
      if( GetWindowRect().Width() >= 340 + TANTO + 20)
      {
	      m_SlotALPHA[i] = 255;
      }
	  m_TEXTUCtrl[i]->Render( p2DRender, CPoint( wndCtrl[i]->rect.left, wndCtrl[i]->rect.top), m_SlotALPHA[i] );
	}
	...
	for( int y = 0; y < 4; y++ )
	{
		for( int i = 0; i < MAX_SLOT_ITEM; i++ )
		{
			...
			{
				if( lpShortcut->m_pTexture )
					p2DRender->RenderTexture( point, lpShortcut->m_pTexture ,m_SlotALPHA[i]);
				if( lpShortcut->m_dwShortcut == SHORTCUT_ITEM )
				{
					CItemBase* pItemBase = g_pPlayer->GetItemId( lpShortcut->m_dwId );
					CItemElem* pItemElem = (CItemElem*)pItemBase;
					...
					if( pItemElem  )
					{
						if( pItemElem->GetProp()->dwPackMax > 1 )
						{
							...
							_stprintf( szTemp, "%d", g_pPlayer?g_pPlayer->m_Inventory.GetItemCount( pItemElem->m_dwItemId ): 0 );
							CSize size = m_p2DRender->m_pFont->GetTextExtent( szTemp );
							p2DRender->TextOut( point.x + 31 - size.cx, point.y + 31 - size.cy + 4, szTemp, D3DCOLOR_ARGB( m_SlotALPHA[i], 133, 255, 138 ));//ademir
						    p2DRender->SetFont( pOldFont );
						}
						DWORD dwGroup = g_pPlayer->m_cooltimeMgr.GetGroup( pItemElem->GetProp() );
				        if( dwGroup )
    ...
    for( int i = 0; i < 10; i++ )
    {
        if( slot1[i] != NULL  )
        {
            slot1[i]->Render( p2DRender, CPoint( wndCtrl[i]->rect.left - 3, wndCtrl[i]->rect.top - 3),m_SlotALPHA[i]);
        }
		if(( m_USEFIRE[i] != NULL ) && ( m_SlotALPHA[i] >= 250 ))
        {
            m_USEFIRE[i]->Render( p2DRender, CPoint( wndCtrl[i]->rect.left, wndCtrl[i]->rect.top), m_USEFireALPHA[i] );
		}
	}
...
HRESULT CWndTaskBar_HUD_SLIDE::RestoreDeviceObjects()
{
	/*for( int i = 0; i<10; i++ )
	{
	m_TEXTUCtrl[i]->SetInvalidate(m_pApp->m_pd3dDevice);
	slot1[i]->SetInvalidate(m_pApp->m_pd3dDevice);
	m_USEFIRE[i]->SetInvalidate(m_pApp->m_pd3dDevice);
	}*/
...
HRESULT CWndTaskBar_HUD_SLIDE::InvalidateDeviceObjects()
{
	/*for( int i = 0; i<10; i++ )
	{
	m_TEXTUCtrl[i]->Invalidate();
	slot1[i]->Invalidate();
	m_USEFIRE[i]->Invalidate();
	}*/
...
HRESULT CWndTaskBar_HUD_SLIDE::DeleteDeviceObjects()
{
	/*for( int i = 0; i<10; i++ )
	{
	m_TEXTUCtrl[i]->DeleteDeviceObjects();
	slot1[i]->DeleteDeviceObjects();
	m_USEFIRE[i]->DeleteDeviceObjects();
	}*/
...
void CWndTaskBar_HUD_SLIDE::OnInitialUpdate( void )
{
    ...
    {
	 m_USEFIRE[i] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "UseSlot.tga" ), 0xffff00ff );
	 m_TEXTUCtrl[i] = CWndBase::m_textureMng.AddTexture( D3DDEVICE, MakePath( DIR_THEME, "WndExpansionSlot01_1.tga" ), 0xffff00ff );
	}
